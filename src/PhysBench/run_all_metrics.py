...
REF_DAILY_ZARR = "gs://weatherbench2/datasets/era5_daily/1959-2023_01_10-full_37-1h-0p25deg-chunk-1-s2s.zarr"
...
def _evaluate_one(
    ...
    ds_ref_daily = open_zarr_anonymous(ref_daily_zarr_path)
    ...
    def _get_daily_t2m(vt: np.datetime64):
        try:
            ...
            if day_time not in _t2m_daily_cache:
                tname = _find_var(ds_ref_daily, T2M_NAMES)
                if tname:
                    t_day = ds_ref_daily[tname].sel(time=day_time, method="nearest").load()
                    t_day = t_day.interp(
                        latitude=ds_ref_full.latitude,
                        longitude=ds_ref_full.longitude,
                        method="linear"
                    )
    ...
    try:
        ...
        if ds_model_full is not None:
            ...
            if has_q and not _use_ref_sp:
                try:
                    ps_model = _get_ps(ds_model_t, ds_static_model, level_dim=model_level_dim, t2m_mean=t2m_mean)
                except Exception as exc:
        ...
        if mode in ("prediction", "model"):
             ...
             if _use_ref_sp:
                 ...
                 ps_ref = _get_ps(ds_ref_t, ds_static, level_dim=ref_level_dim, t2m_mean=t2m_mean)
             else:
        ...
        try:
            ...
            for td_val in avail_tds:
                ...
                if _use_ref_sp:
                    ...
                    try:
                        ...
                        ps_snap_ref = _get_ps(ref_snap_t, ds_static, level_dim=ref_ld_snap, t2m_mean=snap_t2m_mean)
                    ...
                    try:
                        ps_snap = _get_ps(snap, ds_static_model, level_dim=model_level_dim_d, t2m_mean=snap_t2m_mean)
                        dry, water, energy = compute_conservation_scalars(
                            snap, ps_snap, area_model, z_sfc=z_sfc_model,
                            level_dim=model_level_dim_d,
                        )
            ...
            try:
                ...
                for h in hours_model:
                    ...
                    # Compute t2m_mean for this Reference snapshot
                    snap_t2m_mean_e = None
                    try:
                        vt_e = ref_snap.time.values
                    ...
                    try:
                        ps_e = _get_ps(ref_snap, ds_static, level_dim=ref_ld, t2m_mean=snap_t2m_mean_e)
                    except Exception:
                        ps_e = None
...
def run_evaluation(
    ...
    ref_daily_zarr: str = REF_DAILY_ZARR,
    model_name: str = "model",
    ...
    with ProcessPoolExecutor(max_workers=workers) as pool:
        ...
        for idx, (date_str, lead_label, lead_td) in enumerate(work_items, 1):
            fut = pool.submit(
                _evaluate_one,
                prediction_zarr, ref_zarr, ref_daily_zarr, # Pass URL paths, not dataset objects
                date_str, lead_label, lead_td,
...
def main():
    ...
    parser.add_argument(
        "--ref-daily-zarr", type=str, default=REF_DAILY_ZARR,
        help=f"Path to daily Reference Zarr for T2m (default: {REF_DAILY_ZARR})",
    )
    parser.add_argument(
    ...
    run_evaluation(
        ...
        ref_daily_zarr=args.ref_daily_zarr,
